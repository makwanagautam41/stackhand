...
export class StackService {
  ...
  async findOne(id: string) {
    ...
    try {
      if (fs.existsSync(composePath)) {
        ...
        if (parsed?.services) {
          services = Object.entries(parsed.services).map(
            ([name, svc]: [string, any]) => ({
              name,
              image: svc.image,
              ports: svc.ports,
  ...
  async create(dto: {
    ...
    const folderName =
      dto.folderName || dto.name.toLowerCase().replace(/[^a-z0-9-]/g, '-');
  ...
  async updateYaml(id: string, yamlContent: string) {
    ...
    const composePath = path.join(stack.folderPath, 'docker-compose.yml');
    if (!fs.existsSync(composePath))
      throw new BadRequestException('docker-compose.yml not found on disk');
    fs.writeFileSync(composePath, yamlContent, 'utf-8');
    await this.prisma.yamlVersion.create({
      data: {
        stackId: id,
        content: yamlContent,
        message: 'Updated via editor',
      },
    });
    await this.activity.log(
      stack.workspaceId,
      'edit',
      `YAML updated: ${stack.name}`,
    );
  ...
  async composeUp(id: string): Promise<{ stdout: string; stderr: string }> {
    const stack = await this.prisma.stack.findUnique({ where: { id } });
    if (!stack) throw new NotFoundException('Stack not found');
    const result = await this.execComposeWithOutput(stack.folderPath, 'up', [
      '-d',
    ]);
    await this.syncContainers(stack);
    await this.prisma.stack.update({
  ...
  async getLogs(id: string, tail = 200) {
    const stack = await this.prisma.stack.findUnique({ where: { id } });
    if (!stack) throw new NotFoundException('Stack not found');
    return this.execComposeWithOutput(stack.folderPath, 'logs', [
      '--tail',
      String(tail),
    ]);
  }
  ...
  async generateFromTemplate(
    ...
    for (const param of tpl.params.envVars ?? []) {
      yamlStr = yamlStr.replace(
        ...
        overrides[param.key] ?? param.value,
      );
    }
  ...
  getComposeChild(folderPath: string, cmd: string, args: string[] = []) {
    ...
    return spawn('docker', ['compose', '-f', composeFile, cmd, ...args], {
      cwd: folderPath,
    });
  }
  ...
  private async syncContainers(stack: {
    id: string;
    name: string;
    folderPath: string;
  }) {
