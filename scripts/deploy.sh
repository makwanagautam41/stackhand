...
if systemctl is-active --quiet stackhand.service 2>/dev/null; then
  ...
  echo -e "  ${GREEN}Logs: sudo journalctl -u stackhand -f${NC}"
elif command -v pm2 &>/dev/null && pm2 pid stackhand-api &>/dev/null; then
  pm2 restart ecosystem.config.cjs
  echo -e "${GREEN}Deployed! Production running via PM2.${NC}"
else
  echo -e "${GREEN}Build complete. Start production manually:${NC}"
  echo -e "  sudo systemctl start stackhand"
  echo -e "  or: pm2 start ecosystem.config.cjs"
fi
